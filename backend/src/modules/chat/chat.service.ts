...
export class ChatService {
  ...
  async answer(
    ...
    if (hasRelevantHits) {
      ...
      answer = await this.llm.generate(query, contextBlocks, recentQueries);
      data_sufficient = true;
      note = null;
    } else {
      answer = await this.llm.generate(query, [], recentQueries);
  ...
  > {
    ...
    const data_sufficient = hasRelevantHits;
    const note: string | null = hasRelevantHits
      ? null
      : 'Không tìm thấy thông tin liên quan trong cơ sở dữ liệu.';

    let fullAnswer = '';
    const stream = this.llm.generateStream(query, contextBlocks, recentQueries);
