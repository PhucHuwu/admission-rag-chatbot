...
export class IngestService {
  ...
  async ingest(rebuildIndex = false, filterCodes?: string[]) {
    ...
    for (let i = 0; i < allChunks.length; i += batchSize) {
      ...
      console.log(
        `[ingest] Upserted ${Math.min(i + batchSize, allChunks.length)}/${allChunks.length}`,
      );
    }
  ...
  private extractQaFromAdmission(
    schoolName: string,
    code: string,
    text: string,
  ): Array<{ id: string; text: string; intent: string }> {
    ...
    if (lower.includes('phương thức') || lower.includes('xét tuyển')) {
      ...
      if (lower.includes('đánh giá năng lực') || lower.includes('đgnl') || lower.includes('hsa'))
        methods.push('xét đánh giá năng lực');
      if (lower.includes('điểm thi tốt nghiệp') || lower.includes('thpt'))
        methods.push('xét điểm thi THPT');
      if (lower.includes('học bạ')) methods.push('xét học bạ');
      if (lower.includes('chứng chỉ quốc tế') || lower.includes('sat') || lower.includes('act'))
        methods.push('xét chứng chỉ quốc tế');
      if (lower.includes('tài năng') || lower.includes('năng lực vượt trội'))
        methods.push('xét tuyển tài năng');
