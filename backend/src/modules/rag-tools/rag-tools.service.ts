...
export class RagToolsService {
  ...
  private readonly cutoffKeywords = [
    'điểm chuẩn',
    'điểm trúng tuyển',
    'điểm xét tuyển',
    'điểm đầu vào',
  ...
  private readonly admissionMethodKeywords = [
    'phương thức',
    'xét tuyển',
    'tuyển sinh',
    'đề án',
  ...
  analyzeQuery(query: string): QueryAnalysis {
    ...
    if (needsCutoffData || needsTuitionData) {
      ...
      lower.includes('hay hơn')
    ) {
      intent = 'comparison';
    } else if (
      lower.includes('những ngành') ||
      lower.includes('danh sách') ||
      lower.includes('có mấy')
    ) {
      intent = 'list';
    }
  ...
  expandQuery(query: string, analysis: QueryAnalysis): string[] {
    ...
    if (
      analysis.universityCode &&
      !query.toLowerCase().includes(analysis.universityCode.toLowerCase())
    ) {
