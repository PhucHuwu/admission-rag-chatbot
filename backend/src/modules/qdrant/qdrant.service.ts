...
export class QdrantService implements OnModuleInit {
  ...
  private async ensureCollection() {
    try {
      ...
      console.warn(
        `[qdrant] Collection check/create warning: ${err.response?.status || ''} ${err.message || ''} body=${JSON.stringify(err.response?.data)}`,
      );
    }
  }
  ...
  private async collectionExists(name: string): Promise<boolean> {
    try {
      ...
      return (
        res.data?.result?.status === 'green' ||
        res.data?.result?.status === 'yellow' ||
        !!res.data?.result
      );
    } catch {
      return false;
  ...
  async deleteAll() {
    try {
      await axios.delete(`${this.baseUrl}/collections/${this.config.qdrantCollection}`, {
        headers: this.headers,
      });
    } catch {
      void 0;
