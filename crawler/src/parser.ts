...
import { UniversityAdmissionData, UniversityInfo, AdmissionMethod, ProgramInfo } from './types';
...
function detectProgramType(name: string): string {
  ...
  if (lower.includes('liên kết') || lower.includes('hợp tác') || lower.includes('quốc tế'))
    return 'liên_kết_quốc_tế';
...
export async function parseUniversityPage(
    ...
    if (firstSection) {
      ...
      subContents.forEach((sub) => {
        ...
        for (const child of el.children) {
          ...
          const title = titleEl
            ? (titleEl as HTMLElement).innerText.trim().replace(/\s+/g, ' ')
            : '';
          const content = contentEl
            ? (contentEl as HTMLElement).innerText.trim().replace(/\s+/g, ' ')
            : '';
  ...
  for (const method of rawData.methods) {
    ...
    const doiTuongMatch = method.content.match(
      /đối tượng[\s\S]*?(?=quy chế|điều kiện|danh sách|$)/i,
    );
      ...
      for (const row of dataRows) {
        ...
        const nameIdx = row.findIndex(
          (cell: string, i: number) =>
            i !== codeIdx && cell.length > 5 && !cell.includes(';') && !/^\d+$/.test(cell),
        );
        const groupIdx = row.findIndex(
          (cell: string) => cell.includes(';') || /^[A-Z]\d{2}(;|$)/.test(cell),
        );
        const noteIdx = row.findIndex(
          (cell: string, i: number) =>
            i !== codeIdx && i !== nameIdx && i !== groupIdx && cell.length > 0,
        );
