...
for (const file of files) {
  ...
  const hasAdmissionMethods =
    Array.isArray(data.admission_methods) && data.admission_methods.length > 0;
...
if (critical.length > 0) {
  ...
  critical.forEach((r) => {
    ...
    console.log(
      `  Điểm chuẩn: ${r.cutoffMethods} phương thức, PTXT: ${r.admissionMethodsCount} phương thức`,
    );
  });
}
...
console.log(
  `Có điểm chuẩn: ${files.filter((f) => JSON.parse(fs.readFileSync(path.join(outputDir, f), 'utf-8')).cutoff_scores).length}`,
);
