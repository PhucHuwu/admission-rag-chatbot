...
async function inspectPage(url: string, label: string) {
  ...
  try {
    ...
    const sections = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('div.content-page__index-content')).map(
        (el, i) => {
    ...
    sections.forEach((s) => {
      console.log(
        `[${s.index}] id="${s.id}" title="${s.title}" table=${s.hasTable} pdf=${s.hasPdf} chars=${s.textLength}`,
      );
    });
...
(async () => {
  await inspectPage(
    'https://diemthi.tuyensinh247.com/de-an-tuyen-sinh/dai-hoc-bach-khoa-ha-noi-BKA.html',
    'BKA - Bach Khoa Ha Noi',
  );
  await inspectPage(
    'https://diemthi.tuyensinh247.com/de-an-tuyen-sinh/dai-hoc-kinh-te-quoc-dan-KHA.html',
    'KHA - Kinh Te Quoc Dan',
  );
  await inspectPage(
    'https://diemthi.tuyensinh247.com/de-an-tuyen-sinh/hoc-vien-tai-chinh-HTC.html',
    'HTC - Hoc Vien Tai Chinh',
  );
})();
