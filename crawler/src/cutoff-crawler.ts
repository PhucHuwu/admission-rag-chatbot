...
async function parseCutoffPage(page: Page, url: string): Promise<CutoffScores> {
  ...
  for (const method of rawData) {
    ...
    if (colMap['score'] === undefined || colMap['name'] === undefined) {
      console.warn(
        `  Could not detect columns for "${methodName}". Headers: ${headerRow.join(' | ')}`,
      );
      continue;
    }
  ...
  async function processBatch(batch: typeof records) {
    ...
    const promises = batch.map(async (record, idx) => {
      ...
      try {
        ...
        completed++;
        console.log(
          `✅ [${completed}/${records.length}] ${code}: ${cutoffScores.methods.length} methods, ${cutoffScores.methods.reduce((a, m) => a + m.entries.length, 0)} entries`,
        );
      } catch (e) {
        failed++;
