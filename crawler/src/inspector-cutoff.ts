...
async function inspectCutoffPage(url: string, label: string) {
  ...
  try {
    ...
    const tables = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('table')).map((t, i) => {
        const headers = Array.from(
          t.querySelectorAll('thead th, tr:first-child td, tr:first-child th'),
        ...
        const section =
          t.closest('div[class*="content"]')?.className || t.closest('section')?.className || '';
        return {
          index: i,
          headers,
          rowCount: t.querySelectorAll('tr').length,
          sampleRows: rows,
          sectionContext: section.substring(0, 100),
        };
      });
    });
    ...
    const sections = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('div.content-page__index-content')).map((el) => {
        ...
        const title = el.querySelector('h2')
          ? (el.querySelector('h2') as HTMLElement).innerText.trim()
          : '';
        return { id, title };
      });
    });
    ...
    const yearTabs = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('[class*="tab"], [class*="year"], button, a'))
        .filter((el) => /202[0-9]/.test((el as HTMLElement).innerText))
        .map((el) => ({
