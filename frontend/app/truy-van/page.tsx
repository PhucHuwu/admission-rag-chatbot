    ...
    <Shell>
      <section className="rounded-2xl border border-slate-200 bg-white/85 p-4 sm:p-5">
        <h1 className="font-heading text-2xl font-semibold text-slate-900">
          Trang tra cứu thông tin trường
        </h1>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-12">
        <aside className="rounded-2xl border border-slate-200 bg-white/85 p-4 lg:col-span-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <StatusBadge
              ok={!loadingSchools}
              label={loadingSchools ? 'Đang tải' : `${schools.length} trường`}
            />
          </div>
        ...
        <div className="space-y-4 lg:col-span-8">
          <article className="rounded-2xl border border-slate-200 bg-white/85 p-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {selectedSchool ? <StatusBadge ok={true} label={selectedSchool.shortName} /> : null}
            </div>
            {detail?.school ? (
              <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 sm:col-span-2">
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-slate-900">
                    {detail.school['hoc-phi']}
                  </p>
                </div>
              </div>
            ) : (
              <p className="mt-2 text-sm text-slate-500">
                {loadingDetail ? 'Đang tải chi tiết trường...' : 'Chưa có dữ liệu'}
              </p>
            )}
          </article>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white/85 p-4">
            ...
            <div className="mt-3 flex items-center justify-end gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  setPage((p) => (!detail ? p : Math.min(detail.pagination.totalPages, p + 1)))
                }
                disabled={
                  !detail || detail.pagination.page >= detail.pagination.totalPages || loadingDetail
                }
                className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm disabled:opacity-50"
              >
                Sau
          ...
          <article className="rounded-2xl border border-slate-200 bg-white/85 p-4">
            ...
            {detail?.school ? (
              <details className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                <summary className="cursor-pointer text-sm font-medium text-slate-800">
                  Xem đề án tuyển sinh và giới thiệu
                </summary>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Nếu cần đối chiếu thêm, bạn có thể dùng trang Chatbot để hỏi sâu theo từng phương thức
              xét tuyển.
            </p>
          </article>
        </div>
